...
import pyrogram
from info import STICKERS_IDS,PREMIUM_POINT,MAX_BTN, BIN_CHANNEL, USERNAME, URL, ADMINS, LANGUAGES,QUALITIES,YEARS,SEASONS, AUTH_CHANNEL, SUPPORT_GROUP, IMDB, IMDB_TEMPLATE, LOG_CHANNEL, LOG_VR_CHANNEL, TUTORIAL, TUTORIAL2, TUTORIAL3, FILE_CAPTION, SHORTENER_WEBSITE, SHORTENER_API, SHORTENER_WEBSITE2, SHORTENER_API2, IS_PM_SEARCH, QR_CODE, DELETE_TIME
...
from utils import temp, get_settings, is_check_admin, get_status, get_hash, get_name, get_size, save_group_settings, is_req_subscribed, get_poster, get_status, get_readable_time , imdb, replace_words
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
		...
		InlineKeyboardButton('• ɢʀᴏᴜᴘ ᴀᴅᴍɪɴ ᴄᴍᴅ •', callback_data='custom')
	    ],
            [
    ...
    if not spoll:
        ...
        if not files:
            if settings["spell_check"]:
                await delSticker(st)
                ai_sts = await msg.reply_text('<b>ɪᴍᴅʙ ᴀɪ ɪs ᴄʜᴇᴄᴋɪɴɢ ғᴏʀ ᴜʀ sᴘᴇʟʟɪɴɢ ᴘʟᴇᴀsᴇ ᴡᴀɪᴛ..</b>')
                is_misspelled = await ai_spell_check(search)
                if is_misspelled:
                    await ai_sts.edit(f'<b>ɪᴍᴅʙ ᴀɪ sᴜɢɢᴇsᴛᴇᴅ <code>{is_misspelled}</code>\nsᴏ ɪᴍ sᴇᴀʀᴄʜɪɴɢ ғᴏʀ <code>{is_misspelled}</code></b>')
